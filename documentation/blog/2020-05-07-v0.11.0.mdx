...
In `v0.11.x` two new decorators were introduced from the `@moogs/query-graphql` package. The `@Relation` and `@Connection` allow specifying relations on the DTO rather than passing them in as options to the `CRUDResolver`.
...
In `v0.11.x` the `@moogs/query-graph` package is now able to automatically create your resolvers.
