...
In `v0.11.x` two new decorators were introduced from the `@moogs-nestjs-query/query-graphql` package. The `@Relation` and `@Connection` allow specifying relations on the DTO rather than passing them in as options to the `CRUDResolver`.
...
```ts {6}
import { FilterableField, Relation } from '@moogs-nestjs-query/query-graphql';
...
```ts {6}
import { FilterableField, Connection } from '@moogs-nestjs-query/query-graphql';
import { ObjectType, ID, GraphQLISODateTime } from '@nestjs/graphql';
...
In `v0.11.x` the `@moogs-nestjs-query/query-graph` package is now able to automatically create your resolvers.
