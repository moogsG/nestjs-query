...
The `@moogs/core` package provides a number of helper functions to transform or apply queries to a list of items.
...
```ts
import { applyFilter } from `@moogs/core`;
...
```ts
import { applySort, SortDirection, SortNulls } from `@moogs/core`;
...
```ts
import { applyPaging } from `@moogs/core`;
const dtos: TestDTO[] = [
  {first: 'Bob', last: 'Yukon'}
  {first: 'Alice', last: 'Yukon'}
  {first: 'Sally', last: 'Yukon'}
...
```ts
import { applyQuery, SortDirection } from `@moogs/core`;
...
```ts
import { transformFilter, QueryFieldMap, Filter } from `@moogs/core`;
...
```ts
import { transformSort, QueryFieldMap, SortField, SortDirection } from `@moogs/core`;
...
```ts
import { transformQuery, QueryFieldMap, Query, SortDirection } from `@moogs/core`;
...
```ts
import { Filter, getFilterComparisons } from `@moogs/core`;
...
```ts
import { Filter, getFilterOmitting } from `@moogs/core`;
