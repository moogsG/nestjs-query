...
The `@moogs-nestjs-query/core` package provides a number of helper functions to transform or apply queries to a list of items.
...
```ts
import { applyFilter } from `@moogs-nestjs-query/core`;
...
```ts
import { applySort, SortDirection, SortNulls } from `@moogs-nestjs-query/core`;
...
```ts
import { applyPaging } from `@moogs-nestjs-query/core`;
const dtos: TestDTO[] = [
  {first: 'Bob', last: 'Yukon'}
  {first: 'Alice', last: 'Yukon'}
  {first: 'Sally', last: 'Yukon'}
...
```ts
import { applyQuery, SortDirection } from `@moogs-nestjs-query/core`;
...
```ts
import { transformFilter, QueryFieldMap, Filter } from `@moogs-nestjs-query/core`;
...
```ts
import { transformSort, QueryFieldMap, SortField, SortDirection } from `@moogs-nestjs-query/core`;
...
```ts
import { transformQuery, QueryFieldMap, Query, SortDirection } from `@moogs-nestjs-query/core`;
...
```ts
import { Filter, getFilterComparisons } from `@moogs-nestjs-query/core`;
...
```ts
import { Filter, getFilterOmitting } from `@moogs-nestjs-query/core`;
