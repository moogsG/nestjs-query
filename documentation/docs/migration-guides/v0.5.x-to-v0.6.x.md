...
import { InjectTypeOrmQueryService } from '@moogs-nestjs-query/query-typeorm';
...
In the previous version of `@moogs-nestjs-query` the `TypeOrmQueryService` translated between the DTO and Entity. For a more in-depth description see [#41](https://github.com/doug-martin/nestjs-query/issues/41)
...
```ts
import { AssemblerQueryService, QueryService } from '@moogs-nestjs-query/core';
import { InjectTypeOrmQueryService } from '@moogs-nestjs-query/query-typeorm';
import { TodoItemDTO } from './todo-item.dto';
