...
The core of `nestjs-query` is the `Query`, it is used by `@moogs-nestjs-query/query-graphql`, `@moogs-nestjs-query/query-typeorm`
 `@moogs-nestjs-query/query-sequelize` and `@moogs-nestjs-query/query-mongoose`.
